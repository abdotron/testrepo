...
def agents_average(ordered_display,selected_signs):
	avereged=[] #this is the returned variable that contains the summarized data
	out_signs=0
...
#	print last_second
	...
	while i <len(ordered_display):
		...
		#print i
				...
				if ordered_display[j].split(',')[0]=='x' and ordered_display[j].split(',')[2]==current_sign:
					countx=countx+1
				elif ordered_display[j].split(',')[0]=='u' and ordered_display[j].split(',')[2]==current_sign:
					countu=countu+1
				elif ordered_display[j].split(',')[0]=='-x' and ordered_display[j].split(',')[2]==current_sign:
					count_x=count_x+1
				else :
					out_signs=out_signs+1 #dummy variable			
					
			j=j+1
		hi=check_division2(countx,count_x,countu)		
		#print str(hi) + str(countx)+','+str(count_x)+','+str(countu)
	#########################number of agents to total agents 10 sub agents to total agents
########################adding redundant seconds #############3		
#		if i< len(ordered_display):
#			for k in range(last_display_second,current_second):
#				#print str(current_second)+",+str(k)
#				
#				if 5>= hi[2] >= 1  :
#					avereged.append(hi[0]+hi[1]+","+ordered_display[i-1][3:])
#		
#				else:
#					avereged.append(ordered_display[i-1])
